...
3) Add `--version` flag to `superloop.sh` and print a version string.
4) Add a JSON schema file describing `.superloop/config.json` (minimal, but accurate to current fields).
...
Verification:
- `./superloop.sh --version` prints a version string.
- `./superloop.sh validate --repo .` succeeds on the repo's config.
