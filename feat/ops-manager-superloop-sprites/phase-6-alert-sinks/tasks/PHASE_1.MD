...
1. [x] Add alert dispatch script that consumes newly emitted escalation entries.
2. [x] Integrate dispatch invocation into reconcile flow without breaking existing reconcile exit semantics.
3. [x] Ensure dispatch is idempotent across repeated reconciles.

## P1.3 Sink Adapters and Telemetry
1. [x] Implement adapter support for `webhook`, `slack`, and `pagerduty_events`.
2. [x] Persist delivery attempt/outcome telemetry under `.superloop/ops-manager/<loop>/telemetry/`.
