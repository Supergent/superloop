...
1. [x] Add an autonomous execution path that dispatches only eligible intents via existing `ops-manager-control.sh` behavior.
2. [x] Preserve trace/idempotency propagation and deterministic execution status (`executed`, `ambiguous`, `failed`) across transports.
3. [x] Ensure autonomous dispatch failures are reason-coded and do not execute out-of-policy intents.

## P1.5 Operator Visibility and Runbook
1. [x] Extend fleet status/reason surfaces for autonomous remediation outcomes and safety-gate decisions.
2. [x] Update `docs/ops-manager-runbook.md` with autonomous mode operating workflow, kill-switch usage, and fallback to explicit manual handoff.
3. [x] Document operator governance expectations for when autonomous mode should be enabled/disabled.
...
2. [x] Add/extend transport parity regressions in `tests/ops-manager-service.bats` and `tests/ops-manager-visibility.bats` for autonomous execution semantics.
3. [x] Add regressions for deterministic reason-code behavior under partial autonomous execution failures.
...
3. [x] Updated docs/runbook match implemented autonomous policy fields, execution semantics, and operator workflow.
